...
d = d %>% 
  mutate(treat = ifelse(race == 'b', 1, 0)) 
...
d_bal = d %>% 
  select(treat, education, ofjobs, yearsexp, computerskills, h, l, city) #COMPLETE
...
lm_simple = lm_robust(formula = call ~ treat, data = d)#COMPLETE CODE
...
#by 3.2% points compared to having a white sounding name on your CV
  ...
  lm_covs = lm_robust(call ~ treat + factor(education) + ofjobs + 
                                           yearsexp + computerskills + h + factor(city), data = d)#COMPLETE CODE
  #wrap in factor to treat a categorical variable vs numerical 
  summary(lm_covs)
