...
CPS1985 <- CPS1985 %>% 
  mutate(log_wage = log(wage)) #create a variable that is the log of wages (Note: Make sure all wages are >0!)

lm1 <- lm(log_wage ~ education + experience, data = CPS1985)
summary(lm1)

#### Q: Interpret the coefficient for education
#one additional year of education is associated with a 9.6% increase in hourly wages, 
...
CPS1985_fit <- CPS1985 %>% 
  mutate(education = mean(education, na.rm=TRUE)) # What does na.rm do?

## Finally, we use the function "predict()" to get the fitted values for log(wages) based on our model lm1
CPS1985_fit <- CPS1985_fit %>% 
  mutate(log_wage_hat_lm = predict(lm1, newdata = .))
