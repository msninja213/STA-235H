...
bt = train(Sales ~ ., data = carseats.train,
           ...
           trControl = trainControl("cv", number = 10),
           nbagg = 100,  
           control = rpart.control(cp = 0)) # Why do we set cp = 0? How large is a tree if the complexity parameter is 0? --> program will create the largest 
...
###################################################################################
### JIIT
#predicting Sales, using the following conditions
#A 10-fold cross validation.
#Tuning the parameter mtry between 3 and 11 variables.
#A minimum node size of 1.

library(parallel)
library(doParallel) #makes code run faster bc creates trees in parallel
...
tuneGrid = expand.grid(
  mtry = 3:11, # Number of random covariates that will test (this should be the number of max predictors you have in your data, if you want to test all)
  splitrule = "variance", # Split rule (IMPORTANT: for regressions use "variance", for classification use "gini")
  min.node.size = 1 # You can vary this (e.g. include c(5,10) if you want to test different min node sizes) or just live it out
)
