...
d_s1_bal = d_s1 %>%
  select(-vote02, -contact)
  
  datasummary_balance(~treat_real , data = d_s1_bal, dinm_statistic = "p.value", fmt = 3) #COMPLETE
# Q: Is this study balanced? How can you know?
#- is balanced, the difference in means is small (if 1 or 2 variables are significant it's balanced)


####### 2. Estimate the causal effect
lm1 = lm_robust(vote02 ~ treat_real, data = d_s1) #robust var bc vote is binary
summary(lm1)

lm2 = lm_robust(vote02 ~ treat_real + persons + vote00 + vote98 + 
                  newreg + age + female2 + fem_miss, data = d_s1) #add in covariates 
summary(lm2)
#would expect coefficients to be similar (RCT was well balanced) 
...
#not a causal effect - contact is not a confounder 

# Q: Interpret this coefficient. Is this a causal effect?

# Let's add some covariates now:
summary(lm_robust(vote02 ~ contact + persons + vote00 + 
                    vote98 + newreg + age + female2, data = d_s1))
...
#caliper allows you to determine how close your matches are --> smaller is more specific 
...
d_m1 %>% 
  select(contact) %>% 
  table() #Same number of treatment and controls!

# Let's check for balance
d_m1_bal = d_m1 %>% 
  select(contact, persons,vote00,vote98,newreg,age,female2)
...
summary(lm_match)
#on average, people who are contacted are 4.9 percentage points more likely vote than
#those who are not contacted, on our matching sample (NEED TO SAY ITS FOR MATCHED)
